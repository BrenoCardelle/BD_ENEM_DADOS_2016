...
function load_tabela (){
 ...
 if [ $arquivo_original == 'empty' ]; then
	...
	echo >> $nome_tabela"_log.txt"
	    ...
	    time (psql -h localhost -d bd_enem_2016_local -U bantunes -c "\copy $nome_tabela from $path1/$diretorio/$arquivo_final$i.csv
    ...
    time (psql -h localhost -d bd_enem_2016_local -U bantunes -c "\copy $nome_tabela from $path1/$arquivo_original
            with delimiter as ',' NULL AS 'null' csv") &>> $nome_tabela"_log.txt"
